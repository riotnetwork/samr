...
void serial_open(void)
{
	...
	if (BOOT_USART_PAD3 != PINMUX_UNUSED)
  {
    ...
    ///* ----------------------------------------------------------------------------------------------
   //* Put Generic Clock Generator 0 as source for Generic Clock Multiplexer 18 (SERCOM0 reference)
   //*/
  GCLK->PCHCTRL[GCLK_CLKCTRL_ID_SERCOM0_CORE_Val].reg = ( GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN_GCLK0 );
  while ( (GCLK->PCHCTRL[GCLK_CLKCTRL_ID_SERCOM0_CORE_Val].reg & GCLK_PCHCTRL_CHEN) == 0 ){
	  // wait for sync
	  }        
